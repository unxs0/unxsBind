#!/bin/bash
#FILE
#	addCIDRAssignment.sh
#PURPOSE
#	Test the CIDR Assignment Tool
#	tBlock tab
#AUTHOR
#	(C) 2008 Hugo Urquiza for Unixservice
#USAGE
#	./addCIDRAssignment.sh <login> <password> <url>
#
...
#Test 1, check maths
cPost="cForClientPullDown=Root&cIPBlock=192.168.0.1/24&gcCommand=Assign IP Block&gcFunction=tBlockTools&uMathCheck=1";
...
cPost="cForClientPullDown=Root&cIPBlock=192.168.0.1/23&gcCommand=Assign IP Block&gcFunction=tBlockTools&uMathCheck=1";
wget --load-cookies /tmp/iDNS.cookies --no-check-certificate -O /tmp/iDNS.tBlock.out -q --post-data="$cPost" $cURL;
...
cPost="cForClientPullDown=Root&cIPBlock=192.168.0.1/22&gcCommand=Assign IP Block&gcFunction=tBlockTools&uMathCheck=1";
wget --load-cookies /tmp/iDNS.cookies --no-check-certificate -O /tmp/iDNS.tBlock.out -q --post-data="$cPost" $cURL;
...
cPost="cForClientPullDown=Root&cIPBlock=192.168.0.1/32&gcCommand=Assign IP Block&gcFunction=tBlockTools&uMathCheck=1";
wget --load-cookies /tmp/iDNS.cookies --no-check-certificate -O /tmp/iDNS.tBlock.out -q --post-data="$cPost" $cURL;
...
echo;
echo "Test 2/2 Add CIDR block assigment";

cPost="cForClientPullDown=Root&cIPBlock=192.168.0.1/24&gcCommand=Assign IP Block&gcFunction=tBlockTools";
wget --load-cookies /tmp/iDNS.cookies --no-check-certificate -O /tmp/iDNS.tBlock.out -q --post-data="$cPost" $cURL;
...
if [ "$uZone" != "" ] & [ "$uBlock" != "" ]; then
	echo "Test 2 seems OK. Please review uZone and uBlock values below:";
	echo "uZone=$uZone";
	echo "uBlock=$uBlock";
	...
	echo;
	echo "tBlock.cLabel=";
	ForeignKey "tBlock" "cLabel" "uBlock" $uBlock;
	echo;
else
	echo "Test 2 failed. Either the tZone or tBlock record was not created.";
	echo "Further information:";
	echo "uZone=$uZone";
	echo "uBlock=$uBlock";
fi
